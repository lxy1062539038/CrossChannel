...
import "contract/libs/erc20.sol";
/**
 * contract HTLC_ERC20 -> A Hash Time Lock Contract
 *
 * LockContract
 * event log_fund, log_withdraw, log_refund
 * modifier is_token_transferable, future_endtime, is_locked_contract_exist, 
 * check_secret_hash_mathces, withdrawable, refundable
 *
 * function fund()
 * function withdraw()
 * function refund()
 * function get_locked_contract()
 * function have_locked_contract()
 */
...
contract HTLC_ERC20 {
    ...
    modifier check_secret_hash_mathces (bytes32 locked_contract_id, string memory preimage) {
     ...
     * 
     * @param token ERC2- Token contract address.
    ...
    function fund (
        ...
        locked_contract_id = sha256(abi.encodePacked(token, secret_hash, recipient, sender, endtime, amount));
     ...
     */
    function withdraw (bytes32 locked_contract_id, string memory preimage) external is_locked_contract_exist (locked_contract_id) check_secret_hash_mathces (locked_contract_id, preimage) withdrawable(locked_contract_id) returns (bool) {
        ...
        ERC20(locked_contract.token).transfer(locked_contract.recipient, locked_contract.amount);
     ...
     * @param locked_contract_id of HLTC to refund.
     * 
     * @return bool true on success or false on failure.
     */
    function refund (bytes32 locked_contract_id) external is_locked_contract_exist (locked_contract_id) refundable (locked_contract_id) returns (bool) {
        ...
        ERC20(locked_contract.token).transfer(locked_contract.sender, locked_contract.amount);
    ...
    /**
     * @dev Get HTLC ERC20 contract detials.
     * 
     * @param locked_contract_id of HTLC ERC20 to get details
     *
     * @param id token secret_hash recipient sender endtime amount withdrawn refunded preimage locked HTLC ERC20 contract data's.
     */
     ...
     *
     * @param locked_contract_id of HTCL to find it exists.
     *
     * @return exists boolean true of false.
     */
